              ...
              <div className="container mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-5 xl:grid-cols-5 gap-0 md:gap-3 lg:gap-2 xl:gap-4">
                  <div className="details col-span-2 px-3 xl:px-0 pt-5 xl:pt-10">
                    ...
                    <h3 className='text-4xl md:text-[2rem] leading-[.9] pr-8 whitespace-pre-line uppercase font-db font-bold'>{contestGroup.contestLabel}</h3>
                  ...
                  <div className="image-thumb md:col-span-3 xl:col-span-3">
                    <LazyLoadImage 
                    alt={contestGroup.winner.imageName + ' โดย ' + contestGroup.winner.prefix+contestGroup.winner.name+' '+contestGroup.winner.lastname} 
                    title={contestGroup.winner.imageName + ' โดย ' + contestGroup.winner.prefix+contestGroup.winner.name+' '+contestGroup.winner.lastname}  
                    src={'https://assetwise.co.th/photocontest/'+ contestGroup.key +'/awards/'+ contestGroup.winner.url+'.jpg'} />
                  </div>
                </div>
              </div>
            </Transition>
              ...
              <div className="awards-listed pb-5">
                ...
                <ResponsiveMasonry >
                  ...
                  {contestGroup.awards.map((item,i) => (
                    <LazyLoadImage title={item.imageName + ' โดย ' + item.prefix+item.name+' '+item.lastname} alt={item.imageName + ' โดย ' + item.prefix+item.name+' '+item.lastname} key={i} className={'cursor-pointer hover:opacity-80 transition-all'} src={ contestGroup.path + '/awards/thumbs/' + item.url +'.jpg' } onClick={ () => slideClickHandle(item, i, contestGroup.path + '/awards/' + item.url +'.jpg')} />
                  ))}
                  </Masonry>
                </ResponsiveMasonry>
              </div>
